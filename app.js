...
function submitModal(){let m=modalType;if(m==='account'){let name=val('aName').trim(),type=val('aType'),opening=+val('aOpening')||0;if(!name)return alert('Hesap adı gerekli.');let a={id:uid(),name,type,opening};if(type==='credit'){a.cardLimit=+val('aLimit')||0;a.statementDay=+val('aStatementDay')||1;a.dueDay=+val('aDueDay')||10;if(a.cardLimit<=0)return alert('Kredi kartı limiti 0’dan büyük olmalı.')}data.accounts.push(a);save();closeModal()}
...
function openAccount(){openModal('Yeni Hesap','account',`<label>Hesap adı</label><input id="aName" placeholder="Ziraat Bankası"><label>Tür</label><select id="aType" onchange="toggleAccountFields()"><option value="bank">Banka</option><option value="cash">Nakit</option><option value="credit">Kredi Kartı</option><option value="investment">Yatırım</option><option value="debt">Borç</option></select><label>Açılış bakiyesi</label><input id="aOpening" type="number" step="0.01" placeholder="0"><div id="creditFields" style="display:none"><label>Kredi kartı limiti</label><input id="aLimit" type="number" step="0.01" placeholder="50000"><label>Ekstre kesim günü</label><input id="aStatementDay" type="number" min="1" max="31" value="1"><label>Son ödeme günü</label><input id="aDueDay" type="number" min="1" max="31" value="10"></div>`);toggleAccountFields()}
...
/* FinPal 5.2 — Güvenli Cihaz Aktarımı (yerel, üçüncü taraf bulut yok) */
