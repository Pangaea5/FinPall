...
function openAccount(){openModal('Yeni Hesap','account',`<label>Hesap adı</label><input id="aName" placeholder="Ziraat Bankası"><label>Tür</label><select id="aType" onchange="toggleAccountFields()"><option value="bank">Banka</option><option value="cash">Nakit</option><option value="credit">Kredi Kartı</option><option value="investment">Yatırım</option><option value="debt">Borç</option></select><div class="muted" style="margin:8px 0 12px">Yeni hesap 0 TL ile başlar. Bakiye ve kart borcu eklediğin gerçek işlemlerden hesaplanır.</div><div id="creditFields" style="display:none"><label>Kredi kartı limiti (isteğe bağlı)</label><input id="aLimit" type="number" min="0" step="0.01" placeholder="Boş bırakabilirsin"><label>Ekstre kesim günü</label><input id="aStatementDay" type="number" min="1" max="31" value="1"><label>Son ödeme günü</label><input id="aDueDay" type="number" min="1" max="31" value="10"></div>`);toggleAccountFields()}
